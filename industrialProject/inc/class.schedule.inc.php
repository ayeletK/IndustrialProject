    ...
    public function AddNewSchedule()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (empty($_POST["schedule_id"])) {
                ...
                if (!preg_match("/^[0-9_\- ]*$/",$schedule_id)) {
                    $data_correct = 0;
                }
            }//close the else
            ...
            // assumption: account it chose by drop down list connected to db 
            // hence only existing not expired account can be chosen
            //====================================================            
            //todo: check that the accountname doesn't allready exist in database
            if (empty($_POST["account_name"])) {
                $data_correct = 0;
            ...
            if ($data_correct == 1){
                ...
                $query = "SELECT count(*) From `schedules` WHERE ".__schedules_tl_schedule_name." LIKE '$schedule_name' OR ".__schedule_tl_schedule_id." LIKE '$schedule_id'";
                $is_data_exist = mysql_query($query) or die(mysql_error());
